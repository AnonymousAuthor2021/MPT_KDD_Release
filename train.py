...
def test_finetune(epoch, valid_loader, model, output_layer, criterion, sw, opt):
    ...
    print("Epoch, loss, f1", epoch, epoch_loss_meter.avg, epoch_f1_meter.avg)
    return epoch_loss_meter.avg, epoch_f1_meter.avg
...
if __name__ == "__main__":
    ...
    if args.cv:
        ...
        output = open("finetune_temp_result.file", "a")
        output.write("finetune" + args.emb_path + "\n")
        output.write(str(time) + args.dataset + " ")
        #ret, std = task.train()
        ret, std = np.mean(f1), np.std(f1, ddof=1)
        #print("acc:", ret, std)
        output.write(str(ret) + " " + str(std) + "\n")
        output.close()
        #print(f"Mean = {np.mean(f1)}; Std = {np.std(f1)}")
        print("Mean, std", np.mean(f1), np.std(f1))
    else:
        args.gpu = args.gpu[0]
        main(args)
    # import optuna
