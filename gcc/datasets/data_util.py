...
def labeled_motif_batcher():
    def batcher_dev(batch):
        graph_q, label, label_degree = zip(*batch)
        graph_q = dgl.batch(graph_q)
        return graph_q, torch.FloatTensor(label), torch.FloatTensor(label_degree)
...
def create_node_classification_dataset(dataset_name):
    print(dataset_name)
    ppd_dataset = set({"loan", "debt", "investor", "agent", "dx"})
    if dataset_name in ppd_dataset:
        return Edgelist(
            "data/struc2vec/ppd",
            {
                "dx": "dx", 
...
def _rwr_trace_to_dgl_graph(
    g, graph_idx, seed, trace, positional_embedding_size, entire_graph=False
):
    ...
    #print(subg.ndata)
    #exit(0)
    subg.ndata['graph_idx'] = torch.zeros(subg.number_of_nodes(), dtype=torch.int64)
    subg.ndata['graph_idx'][0] = graph_idx
    subg.ndata['node_idx'] = torch.zeros(subg.number_of_nodes(), dtype=torch.int64)
    # print(graph_idx, seed)
    subg.ndata['node_idx'][0] = torch.from_numpy(np.array(seed)).to(subg.ndata['node_idx'])
    #subg.ndata['graph_idx'] = graph_idx
    #subg.ndata['node_idx'] = seed
    return subg
...
def _add_undirected_graph_positional_embedding(g, hidden_size, retry=10):
    ...
    # print(len(x.float()), len(x.float()[0]), x.float(), g.edges())
    # edges = g.edges()
    # exit()
    # get python motif embedding
    #write file, exec exe, get_file
    return g
