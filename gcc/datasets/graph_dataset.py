...
graph_motif_list = {}
def load_motif(name):
    #names = ["livejournal", "facebook", "dblp_netrep", "dblp_snap", "academia", "pokec"]
    names = [name]
    counts = []
...
def calculate_degree(G):
    nums = []
    for i in G.nodes():
        nums.append(len(G.in_edges(i)[0]))
    #print(nums)
    return np.mean(nums)
...
class LoadBalanceGraphDataset3(torch.utils.data.IterableDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.graph_transform:
            graph_q = self.graph_transform(graph_q)
        #print("here", graph_q, self.motif_dict[(graph_idx, node_idx)], calculate_degree(graph_q))
        return graph_q, self.motif_dict[(graph_idx, node_idx)], calculate_degree(graph_q)
